...
training_loader = torch.utils.data.DataLoader(training_dataset, batch_size=128, shuffle=True) # Batch size of 100 i.e to work with 100 images at a time
...
#plt.title("Adam vs Adamw weight decay")
#plt.xlabel('Epoch')
#plt.ylabel('Loss')
for opt in optimizers:
    if opt == 'adam':
        wds = [0,0.1,0.01, 0.001]
    elif opt == 'adamw':
        wds = [0,0.1,0.01, 0.001]
    else:
        print("ERROR: Optimizer", opt, "Not found")
    plt.title("Loss vs Weight decay: " + opt)
    plt.xlabel('Epoch')
    ...
    for wd in wds:
        ...
        if opt == 'adam':
            optimizer = torch.optim.Adam(params, lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=wd, amsgrad=False)
        elif opt == 'adamw':
            optimizer = torch.optim.AdamW(params, lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=wd, amsgrad=False)
        else:
            print("ERROR: Optimizer", opt, "Not found")
        ...
        label = "val" + str(wd)
        plt.plot(val_running_loss_history, label=label)
        label = "train" + str(wd)
